...
contract CRNT is Context, IBEP20, IBEP20Metadata, Ownable {
    ...
    constructor(uint _maxSupply, string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
    ...
    function transfer(
        ...
        uint256 tenPercentAmount = (amount * 100) / 1000; // Calculate 10% tax amount
        uint256 fivePercent = tenPercentAmount / 2; //  5% tax amount for liquidity and 5% tax amount for burn
        ...
        IBEP20(address(this)).approve(address(zapper),fivePercent);
        zapper.zapInToken(address(this), fivePercent, poolAdd, router, owner()); //add to pool
    ...
    function transferFrom(
        ...
        if((from == address(this) && to == address(zapper)) || to == poolAdd) {
        // if(to == poolAdd) {
            console.log("HERER");
            console.log(to == router);
        _transfer(from, to, amount);
        }else {
            console.log("HERER22222");
